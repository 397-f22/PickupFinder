...
const EventCard = ({ event, users, eventId, toggleEvent, currentUser }) => {
    ...
    const bt2text = isCurrentUserInEvent ? (isCurrentUserOrganizer ? " Discard" : " Unjoin") : " Join"
    const bt2icon = isCurrentUserInEvent ? (isCurrentUserOrganizer ? "bi bi-trash3-fill" : "bi bi-box-arrow-right") : "bi bi-box-arrow-in-left"
    ...
    return (
        <div>
            ...
            <Card style={{ width: `${cardwidth}` }} className="m-1">
                ...
                <Card.Footer>
                    <Button className="bi bi-people-fill" variant="primary" onClick={() => {
                        openEventDetailsModal();
                    }}><p className="d-none d-lg-inline"> See attendees</p></Button>
                    {currentUser.uid !== 'guest' &&
                        (<Button className={bt2icon} variant={`${isCurrentUserInEvent ? (isCurrentUserOrganizer ? "danger" : "danger") : "primary"} ms-3`}
                            disabled={isEventAtCapacity}
                            onClick={() => {
                                toggleEvent(event, eventId, isCurrentUserOrganizer);
                            }}><p className="d-none d-lg-inline">{bt2text}</p></Button>)
                    }
                </Card.Footer>
            </Card >
        </div>
